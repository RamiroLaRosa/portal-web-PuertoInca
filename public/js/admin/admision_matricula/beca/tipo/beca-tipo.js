document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            eReq.value = btn.dataset.requisito || '';
            eIcono.value = btn.dataset.icono || '';
            eActive.checked = parseInt(btn.dataset.active, 10) === 1;

            // RUTA PUT con el id del recurso
            formEdit.action = window.becaTipoUpdateRoute.replace('__ID__', id);
            editModal.show();
        });
    });

    // -------- Paginación tabla beca tipo ----------
    const tableBody = document.querySelector('.beca-tipo-table tbody');
    if (tableBody) {
        ...
        function createPagination() {
            let pagination = document.querySelector('.beca-tipo-pagination');
            if (pagination) pagination.remove();
            pagination = document.createElement('nav');
            pagination.className = 'beca-tipo-pagination mt-3';
            ...
            if (table) {
                const prevPaginations = table.parentElement.querySelectorAll('.beca-tipo-pagination');
