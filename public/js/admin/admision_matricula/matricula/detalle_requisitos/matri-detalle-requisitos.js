document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Importante: la URL PUT tiene el ID
            formEdit.action = window.matriDetalleRequisitosUpdateRoute.replace('__ID__', id);
            editModal.show();
        });
    });

    // -------- Paginación tabla matricula detalle requisitos ----------
    const tableBody = document.querySelector('.matri-detalle-requisitos-table tbody');
    if (tableBody) {
        ...
        function renderPage(page) {
            ...
            if (pageRows.length === 0) {
                ...
                noDataRow.innerHTML = '<td colspan="4" class="text-center text-muted">Sin registros.</td>';
                tableBody.appendChild(noDataRow);
            }
        }

        function createPagination() {
            let pagination = document.querySelector('.matri-detalle-requisitos-pagination');
            if (pagination) pagination.remove();
            pagination = document.createElement('nav');
            pagination.className = 'matri-detalle-requisitos-pagination mt-3';
            ...
            if (table) {
                const prevPaginations = table.parentElement.querySelectorAll('.matri-detalle-requisitos-pagination');
