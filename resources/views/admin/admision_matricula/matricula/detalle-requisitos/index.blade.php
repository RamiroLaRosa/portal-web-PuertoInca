...
<head>
    ...
    <title>Requisitos de Matricula (Detalle)</title>
    ...
    <style>
        /* Cabecera fija dentro del contenedor scroll */
        .sticky-head th {
            position: sticky;
            top: 0;
            z-index: 2;
        ...
        /* Layout tabla */
        .matri-detalle-requisitos-table {
            table-layout: fixed;
        }

        .matri-detalle-requisitos-table th,
        .matri-detalle-requisitos-table td {
            vertical-align: middle;
        }

        /* Truncado para descripción */
        .truncate-text {
            max-width: 100%;
            overflow: hidden;
...
<body data-titulo="Registro de información sobre Detalle de Requisitos de Matricula" class="has-sidebar">
            ...
            <div class="card roles-card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="roles-toolbar d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                        <h5 class="mb-0 fw-bold">Registro de información sobre Detalle de Requisitos de Matricula</h5>
                            ...
                            <tbody>
                                @forelse($items as $row)
                                    <tr>
                                        ...
                                        {{-- Título (de la cabecera a la que pertenece el detalle) --}}
                                        <td class="text-muted fw-semibold">
                                            {{ $row->requisito?->titulo ?? '—' }}
                                        </td>
                                        ...
                                        {{-- Ícono (font-awesome) de la cabecera --}}
                                        <td class="text-center">
                                            @if ($row->requisito?->icono)
                                                <i class="{{ $row->requisito->icono }} fa-lg"></i>
                                            @else
                                                —
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="d-inline-flex gap-1 actions">
                                                <button class="btn btn-warning btn-sm text-white btn-edit"
                                                    ...
                                                    data-req="{{ $row->matricula_requisitos_id }}"
                                                ...
                                                <button class="btn btn-danger btn-sm btn-delete" title="Eliminar"
                                                    data-url="{{ route('matri-detalle-requisitos.destroy', $row) }}"
                                                    data-title="{{ e($row->requisito->titulo ?? 'este registro') }}"
        ...
        <div class="modal fade" id="modalNuevo" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form action="{{ route('matri-detalle-requisitos.store') }}" method="POST">
                        @csrf <input type="hidden" name="_from" value="create">
                        <div class="modal-header">
                            <h5 class="modal-title" style="color:#2563eb;">Nuevo detalle</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label" style="color:#2563eb;">Título (cabecera) *</label>
                                <select name="matricula_requisitos_id" class="form-select" required>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form id="formEditar" action="#" method="POST">
                        @csrf @method('PUT')
                        <div class="modal-header">
                            <h5 class="modal-title" style="color:#2563eb;">Editar detalle</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label" style="color:#2563eb;">Título (cabecera) *</label>
                                <select id="editReq" name="matricula_requisitos_id" class="form-select" required>
                                    @foreach ($requisitos as $r)
                                        <option value="{{ $r->id }}">{{ $r->titulo }}</option>
                                    @endforeach
                                </select>
    ...
    <!-- js general -->
    @vite('resources/js/admin/admision_matricula/matricula/detalle_requisitos/matri-detalle-requisitos.js')
