{{-- resources/views/admin/admision_matricula/admision/proceso/index.blade.php --}}
...
<head>
    ...
    <style>
        ...
        .proceso-table th, .proceso-table td { vertical-align: middle; }
        ...
        /* Acciones del mismo tamaño */
        .actions .btn-icon, .actions .btn {
            width: 34px;
            height: 34px;
            display: inline-flex;
            align-items: center;
        ...
        /* Ancho extra para descripción en pantallas grandes */
        @media (min-width: 992px) {
            .truncate-text { max-width: 720px; }
        }
        @media (max-width: 991.98px) {
            .truncate-text { max-width: 320px; }
        }
    </style>
</head>

<body data-titulo="Registro de información sobre Proceso de Admisión" class="has-sidebar">
            ...
            <div class="card roles-card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="roles-toolbar d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                        <h5 class="mb-0 fw-bold">Registro de información sobre Proceso de Admisión</h5>
                            ...
                            <tbody>
                                @forelse($items as $row)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="truncate-text" title="{{ $row->descripcion }}">
                                                {{ $row->descripcion }}
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            @if ($row->paso && $row->paso->icono)
                                                <i class="{{ $row->paso->icono }}"></i>
                                            @else
                                        ...
                                        <td class="text-center">
                                            <div class="d-inline-flex gap-1 actions">
                                                <button
                                                    class="btn btn-warning btn-icon btn-sm text-white btn-edit"
                                                    title="Editar"
                                                    data-id="{{ $row->id }}"
                                                    data-paso="{{ $row->admision_paso_id }}"
                                                ...
                                                <button
                                                    ...
                                                    title="Eliminar"
                                                    data-url="{{ route('admin-proceso.destroy', $row) }}"
                                                    data-title="{{ e($row->paso->paso ?? 'este registro') }}"
                ...
                <div class="modal-content">
                    <form action="{{ route('admin-proceso.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label" style="color:#2563eb;">Paso *</label>
                                <select name="admision_paso_id" class="form-select" required>
                                    <option value="" hidden>Seleccione…</option>
                                    @foreach ($pasos as $p)
                                        <option value="{{ $p->id }}">{{ $p->paso }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                ...
                <div class="modal-content">
                    <form id="formEditar" action="#" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label" style="color:#2563eb;">Paso *</label>
                                <select id="editPaso" name="admision_paso_id" class="form-select" required>
                                    @foreach ($pasos as $p)
                                        <option value="{{ $p->id }}">{{ $p->paso }}</option>
                                    @endforeach
                                </select>
    ...
    <!-- js general -->
    @vite('resources/js/admin/admision_matricula/admision/proceso/proceso.js')
    ...
    <script>
        // Modal eliminar: setea action y nombre
