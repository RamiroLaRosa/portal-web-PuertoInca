...
<body data-titulo="Registro de información sobre Submódulos" class="has-sidebar">
            ...
            <div class="card roles-card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="roles-toolbar d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                        <h5 class="mb-0 fw-bold">Registro de información sobre Submódulos</h5>
                    ...
                    <div class="table-responsive">
                        <table id="tabla-submodulos" class="table table-bordered content-table" data-page-size="5">
                            <!-- ← 5 filas por página -->
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                @forelse($submodulos as $s)
                                    <tr>
                                        <td>{{ $s->id }}</td>
                                        <td class="text-muted fw-semibold">{{ $s->header?->nombre ?? '—' }}</td>
                                        <td class="text-muted fw-semibold">{{ $s->nombre }}</td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <select class="form-select form-select-sm sub-visibility"
                                                    data-id="{{ $s->id }}" aria-label="Cambiar visibilidad">
                                                    <option value="1" @selected($s->is_active)>Visualizar
                                                    </option>
                                                    <option value="0" @selected(!$s->is_active)>Ocultar</option>
                                                </select>
                                            </div>
    ...
    <script>
        window.subUpdateUrlBase = "{{ url('/admin/inicio/submodulos') }}";
    </script>
    ...
    @vite(['resources/js/admin/seguridad/submodulo/submodulo.js'])
