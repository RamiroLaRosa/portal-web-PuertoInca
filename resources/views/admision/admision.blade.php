    ...
    <main class="md:pl-20 pt-16">
        ...
        <section id="resultados" class="py-20 bg-white">
            <div class="container mx-auto px-4 md:px-12">
                <div class="mb-12">
                    <h2 class="text-4xl md:text-5xl font-bold mt-2 text-brand-navy">Resultados de Admisión</h2>
                </div>
                        ...
                        <div class="space-y-4 p-4">
                            @foreach ($filas as $row)
                                <div class="bg-brand-gray rounded-lg p-4">
                                    <h4 class="font-semibold text-brand-navy mb-4">{{ $row['p']->nombre }}</h4>
                                    <div class="flex justify-between">
                                        <div class="text-center">
                                            <p class="text-sm text-brand-navy/70 mb-2">Turno Diurno</p>
                                            @if ($row['diurno'])
                                                <a href="{{ $row['diurno']->documento_url }}" target="_blank"
                                                    rel="noopener"
                                        ...
                                        <div class="text-center">
                                            <p class="text-sm text-brand-navy/70 mb-2">Turno Nocturno</p>
                                            @if ($row['nocturno'])
                                                <a href="{{ $row['nocturno']->documento_url }}" target="_blank"
                                                    rel="noopener"
                                                    class="inline-flex items-center justify-center bg-brand-sky/10 hover:bg-brand-sky/20 p-2 rounded-lg">
        ...
        <section id="modalidades" class="py-20 bg-brand-gray">
            <div class="container mx-auto px-4 md:px-12">
                <div class="mb-12">
                    <h2 class="text-4xl md:text-5xl font-bold mt-2 text-brand-navy">Modalidades de Admisión</h2>
                </div>
                    ...
                    @forelse($modalidades as $m)
                        <div class="bg-white rounded-3xl p-8 hover:shadow-lg transition-shadow">
                            <div class="bg-brand-orange/10 p-3 rounded-2xl w-fit mb-6">
                                {{-- El campo "icono" trae la clase FA, p.e. "fa-solid fa-trophy" --}}
                                <i class="{{ $m->icono }} text-brand-orange text-3xl" aria-hidden="true"></i>
                            </div>

                            <h3 class="text-2xl font-bold mb-4 text-brand-navy">{{ $m->titulo }}</h3>
                            <p class="text-brand-navy/70 mb-6 text-justify">{{ $m->descripcion }}</p>
                        </div>
                    @empty
                        <div class="col-span-full text-brand-navy/60">No hay modalidades disponibles por ahora.</div>
                    @endforelse
                </div>
            </div>
        </section>
        ...
        @php
            // Intentamos mapear por nombre; si no, tomamos 1ro y 2do como fallback
        ...
        <section id="requisitos" class="py-20 bg-white">
            <div class="container mx-auto px-4 md:px-12">
                <div class="mb-12">
                    <h2 class="text-4xl md:text-5xl font-bold mt-2 text-brand-navy">Requisitos de Admisión</h2>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    {{-- Columna: Documentos Obligatorios --}}
                    <div class="bg-brand-gray rounded-3xl p-8 shadow-lg">
                        <h3 class="text-2xl font-bold mb-6 flex items-center text-brand-navy">
                            <i data-lucide="file-text" class="h-6 w-6 text-brand-orange mr-3"></i>
                        ...
                        <div class="space-y-4">
                            @forelse(($docOblig->requisitos ?? collect()) as $req)
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 class="font-semibold text-brand-navy">{{ $req->titulo }}</h4>
                                        <p class="text-brand-navy/70 text-sm">{{ $req->descripcion }}</p>
                                    </div>
                                </div>
                            @empty
                                <p class="text-brand-navy/60">Sin registros.</p>
                            @endforelse
                        </div>
                    </div>

                    {{-- Columna: Documentos Adicionales --}}
                    <div class="bg-brand-gray rounded-3xl p-8 shadow-lg">
                        <h3 class="text-2xl font-bold mb-6 flex items-center text-brand-navy">
                            <i data-lucide="plus-circle" class="h-6 w-6 text-brand-orange mr-3"></i>
                        ...
                        <div class="space-y-4">
                            @forelse(($docAdic->requisitos ?? collect()) as $req)
                                <div class="flex items-start gap-3">
                                    <div class="bg-brand-orange/10 p-1 rounded-full mt-1">
                                        <i data-lucide="info" class="h-4 w-4 text-brand-orange"></i>
                                    </div>
                                    <div>
                                        <h4 class="font-semibold text-brand-navy">{{ $req->titulo }}</h4>
                                        <p class="text-brand-navy/70 text-sm">{{ $req->descripcion }}</p>
                    ...
                    <div class="p-8 space-y-8">
                        @forelse($cronograma as $step)
                            <div class="flex items-start gap-6 p-6 bg-brand-gray/50 rounded-2xl">
                                ...
                                <div class="flex-grow">
                                    <h4 class="font-bold text-xl text-brand-navy mb-1">{{ $step->titulo }}</h4>

                                    @if (!empty($step->fecha))
                                        <p class="text-brand-blue font-medium mb-2">{{ $step->fecha }}</p>
                                    @endif

                                    @if (!empty($step->descripcion))
                                        <p class="text-brand-navy/70">{{ $step->descripcion }}</p>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <p class="text-center text-brand-navy/60">No hay fechas publicadas por ahora.</p>
                        @endforelse
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        @foreach ($exonerados as $item)
                            <div class="bg-white rounded-3xl p-8 shadow-lg">
                                <div class="bg-brand-orange/10 p-3 rounded-2xl w-fit mb-6">
                                    <i
                                        class="{{ $item->icono ?: 'fa-solid fa-circle-info' }} text-2xl text-brand-orange"></i>
                                </div>
        ...
        <section class="vacancy-section" id="vacantes">
            <div class="container">
                <div class="section-header">
                    <h2 class="section-title">Número de Vacantes</h2>
                </div>

                <div class="vacancy-card-container">
                    <div class="vacancy-card">
                        <h3 class="card-title">Vacantes Disponibles</h3>
                            ...
                            <div class="space-y-4">
                                @foreach ($vacantes as $row)
                                    <div
                                        ...
                                        <span class="program-name">{{ $row->programa->nombre ?? 'Programa' }}</span>
                                        <span class="program-vacancies">{{ number_format($row->vacantes) }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @endif
        ...
        <section id="proceso" class="py-20 bg-brand-gray">
            <div class="container mx-auto px-4 md:px-12">
                <div class="mb-12">
                    <h2 class="text-4xl md:text-5xl font-bold mt-2 text-brand-navy">Proceso Paso a Paso</h2>
                </div>

                <div class="relative">
                    {{-- línea vertical del timeline (solo desktop) --}}
                    <div class="hidden md:block absolute left-8 top-0 bottom-0 w-0.5 bg-brand-orange/30"></div>
                    ...
                    <div class="space-y-10">
                                        ...
                                        <ul class="space-y-2">
                                            @forelse($paso->procesos as $item)
                                                <li class="flex items-start gap-3">
                                                    ...
                                                    <span class="text-brand-navy/90">{{ $item->descripcion }}</span>
                                                </li>
                                            @empty
                                                <li class="text-brand-navy/60">Sin elementos para este paso.</li>
                                            @endforelse
                                        </ul>
                                    </div>
                                </div>
                        ...
                        @empty
                            <p class="text-brand-navy/60">No hay pasos publicados por ahora.</p>
                        @endforelse
                    </div>
                </div>
            </div>
